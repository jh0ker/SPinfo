...
namespace Awesome
{
    class Program
    {
        const string TOKEN = "yourtokenhere";
        const string MODERATOR_USERNAME = "yourusernamehere";
        ...
        static async void Bot_OnMessage(object sender, MessageEventArgs e)
        {
            ...
            var pokedexRegex = new Regex(@"^(([0-4]+[0-9]+[0-9])|([0-9]+[0-9])|([0-9]))$");
            ...
            try
            {
                subscribed = new List<string>(System.IO.File.ReadAllLines(@"subscribers.txt"));
            }
            catch
            {
                Console.WriteLine("subscribers.txt not found, assuming no subscribers");
            }

            if (e.Message.Text != null)
            {
                Console.WriteLine($"Received a text message in chat {e.Message.Chat.Id} with {e.Message.Text}.");
                msg = e.Message.Text;
                ...
                else if (pokedexRegex.IsMatch(e.Message.Text))
                {
                    ...
                    await botClient.SendTextMessageAsync(
                      chatId: e.Message.Chat,
                      text: "You asked for " + pkmname[0, pokedex],
                      parseMode: ParseMode.Markdown
                    );
                    await botClient.SendTextMessageAsync(
                      chatId: e.Message.Chat,
                      text: "The 100IV CP *without* weather boost in a raid is:\n" + output + "\nThe 100IV CP *with* weather boost in a raid is:\n" + outputwb,
                      parseMode: ParseMode.Markdown
                    );

                    /*
                    // This seems outdated and not needed anymore
                    string searchstring = Convert.ToString(pokedex) + "&";
                    for (int i = 2; i < 81; i++)
                    {
                        cp = Convert.ToInt16(Math.Floor(((((baseAttack + 15) * cpm[i]) * (Math.Sqrt((baseDefense + 15) * cpm[i])) * Math.Sqrt((baseStamina + 15) * cpm[i])) / 10)));
                        searchstring = searchstring + ",cp" + Convert.ToString(cp);
                    }

                    await botClient.SendTextMessageAsync(
                      chatId: e.Message.Chat,
                      text: "The search string for wild 100IV Pokemon is:",
                      parseMode: ParseMode.Markdown
                      );
                    await botClient.SendTextMessageAsync(
                      chatId: e.Message.Chat,
                    ...
                    */
                    ...
                    if (baseStats[3, pokedex] == -1)
                    {
                        await botClient.SendTextMessageAsync(
                          chatId: e.Message.Chat,
                          text: pkmname[0, pokedex] + " is already the highest evolution.",
                          parseMode: ParseMode.Markdown
                        );
                    }
                    else
                    {
                        await botClient.SendTextMessageAsync(
                          chatId: e.Message.Chat,
                          text: pkmname[0, pokedex] + " needs " + baseStats[3, pokedex] + " candys to evolve.",
                          parseMode: ParseMode.Markdown
                        );
                    }
                }
                else if (e.Message.From.Username == MODERATOR_USERNAME && coords.IsMatch(e.Message.Text))
                {
                ...
                else if (e.Message.Text == "/subscribe")
                {
                    ...
                    if (havesubscribed == false)
                    {
                        subscribed.Add(Convert.ToString(e.Message.Chat.Id));
                        System.IO.File.WriteAllLines("subscribers.txt", subscribed);
                        await botClient.SendTextMessageAsync(
                ...
                else
                {
                    await botClient.SendTextMessageAsync(
                      chatId: e.Message.Chat,
                      text: "Commands: \n */cd*: Cooldown table \n */rb*: Raidboss Info \n */rwd*: Raid rewards \n */hs*: Spoof Hotspots \n */subscribe* Subscribe? \n Pokedex number always gives you the 100% CP in a raid",
                      parseMode: ParseMode.Markdown
                    );
